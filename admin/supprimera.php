  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <li class="nav-item">
        ...
        <li class="nav-item">
          <a class="nav-link active" style="font-weight: bold;" aria-current="page" href="supprimera.php">Supprimer un produit-accessoires</a>
        </li>
...
<form method="post">
  ...
  <div class="mb-3">
    ...
    <input type="number" class="form-control" name="idaccessoire" required>
   
  </div>
  <button type="submit" name="va" class="btn btn-danger">Supprimer le produit</button>
</form>
 </div>
...
<?php
  if(isset($_POST['va']))
  {
    if(isset($_POST['idaccessoire']))
    {
    if(!empty($_POST['idaccessoire']) AND is_numeric($_POST['idaccessoire']))
	    {
	    	
	    	$idaccessoire = htmlspecialchars(strip_tags($_POST['idaccessoire']));
          
          try 
          {
            supprimera($idaccessoire);
          }
          catch (Exception $e) 
          {
          	$e->getMessage();
